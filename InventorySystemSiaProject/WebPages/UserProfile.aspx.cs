        ...
        private string FormatActivityLog(ActivityLog log)
        {
            ...
            try
            {
                if (log.Action != null && log.Action.ToLower().Contains("create"))
                {
                    ...
                    sb.Append("<div class='activity-row'><span class='activity-label'>Supplier Id:</span><span class='activity-value'>" + detailsObj["supplierId"] + "</span></div>");
                ...
                else if (log.Action != null && log.Action.ToLower().Contains("update"))
                {
                    ...
                    if (detailsObj["changes"] != null)
                    {
                        var changes = detailsObj["changes"];
                        if (changes["before"] != null && changes["after"] != null)
                        {
                            sb.Append("<div class='activity-before'><b>?? BEFORE</b><br/>" + FormatIngredientDetails(changes["before"]) + "</div>");
                            sb.Append("<div class='activity-after'><b>? AFTER</b><br/>" + FormatIngredientDetails(changes["after"]) + "</div>");
                        }
                    }
                    sb.Append("</div>");
