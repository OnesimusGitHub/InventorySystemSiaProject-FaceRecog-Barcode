            ...
            DateTime? requestDate = null)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(requestId) && requestDate.HasValue)
                {
                    ...
                    string approveUrl = $"{baseUrl}/Handlers/ProcessStockRequestAction.ashx?requestId={requestId}&action=approve&token={token}";
                    string rejectUrl = $"{baseUrl}/Handlers/ProcessStockRequestAction.ashx?requestId={requestId}&action=reject&token={token}";
